...
TIP: If you need to install both these packages, we recommend doing so in separate Python virtual environments (for example, using `python -m venv <virtual-environment-name>`. This avoids any problems when TensorFlow and PyTorch do not play nicely together.
